...
def export_to_html(history):
    """将对话历史导出为 HTML 网页文件，返回文件路径"""
    if not history:
        return None
...
''')
    ...
    temp_dir = tempfile.gettempdir()
    filepath = os.path.join(temp_dir, "chat_history.html")
...
with gr.Blocks(theme=theme, css=css, title="物流智能问答与决策辅助系统") as demo:
    ...
    with gr.Column(elem_classes="main-card"):
        ...
        with gr.Row(elem_classes="export-row"):
            ...
            html_btn = gr.Button("🌐 导出为 HTML", variant="secondary", size="sm")
